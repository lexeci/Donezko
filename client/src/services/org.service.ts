...
class OrgService {
    ...
    async getOrganizationRole(
        ...
        try {
            const response = await axiosWithAuth.get<{ role: OrgRole, status: AccessStatus }>(
    ...
    async deleteOrganization(id: string): Promise<AxiosResponse> {
        try {
            const response = await axiosWithAuth.delete(`${this.BASE_URL}/${id}?organizationId=${id}`);
     ...
     * @example
     * const updatedRole = await orgService.updateRoleOrganization({ id: "user123", role: "admin" });
     */
